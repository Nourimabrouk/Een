    ...
    try:
        ...
        for file_path in folder_path.rglob("*"):
            if file_path.is_file() and is_supported_file(file_path.name):
                ...
                metadata = VISUALIZATION_METADATA.get(
                    filename,
                    {
    ...
    try:
        ...
        all_visualizations.sort(
            key=lambda x: (
                not x.get(
                    "featured", False
                ),  # Featured first (False sorts before True)
...
async def get_visualizations_by_category(category: str, request: Request):
    ...
    try:
        ...
        # Sort
        filtered_visualizations.sort(
            key=lambda x: (not x.get("featured", False), -x.get("modified", 0))
        )
...
async def generate_visualization(request_data: GenerateVisualizationRequest):
    """Generate a new visualization using real DALL-E integration."""
    try:
        ...
        # Generate specialized visualization based on type
        if viz_type == "unity":
            prompt = "Create a transcendental visualization of the Unity Equation: 1+1=1 with φ-harmonic proportions and consciousness field dynamics"
        elif viz_type == "consciousness":
            prompt = "Visualize an 11-dimensional consciousness field with φ-harmonic resonance and unity convergence patterns"
        elif viz_type == "phi_harmonic":
            prompt = "Create a visualization of φ-harmonic resonance in consciousness space with golden ratio spirals"
        elif viz_type == "meta_recursive":
            prompt = "Visualize meta-recursive consciousness evolution patterns with self-referential structures"
        else:
            prompt = f"Generate a consciousness field visualization for {viz_type} mathematics with unity convergence"
        ...
        if result.get("success") and result.get("images"):
            ...
            generated_viz = {
                "src": result["images"][0],  # Use first generated image
                "filename": f'generated_unity_{viz_type}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png',
                ...
                "featured": True,
                "technique": "DALL-E 3 generation with φ-harmonic mathematics",
                "significance": "Live demonstration of unity mathematics generation",
                "created": datetime.now().strftime("%Y-%m-%d"),
                "generated": True,
                "dalle_prompt": result.get("prompt", ""),
            ...
            return {
                ...
                "message": "Visualization generated successfully using DALL-E 3",
                "dalle_result": result,
            }
        else:
            raise Exception(
                f"DALL-E generation failed: {result.get('error', 'Unknown error')}"
            )
