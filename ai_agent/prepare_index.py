...
class EenRepositoryIndexer:
    ...
    INCLUDE_PATTERNS = {
        "*.py", "*.md", "*.html", "*.txt", "*.js", "*.css", 
        "*.json", "*.yml", "*.yaml", "*.r", "*.R"
    }
    
    # Directories to exclude
    EXCLUDE_DIRS = {
        "venv", "een/Lib", "een/Include", "een/Scripts", "__pycache__", 
        ".git", "node_modules", ".pytest_cache", "backups", 
        "logs", "legacy"
    }
    ...
    async def create_embeddings_batch(self, chunks: List[DocumentChunk]) -> List[Dict[str, Any]]:
        ...
        for attempt in range(max_retries):
            try:
                self.logger.info(f"Creating embeddings for {len(texts)} chunks (attempt {attempt + 1})")
...
Remember: You are helping users explore a revolutionary mathematical framework where consciousness, quantum mechanics, and pure mathematics converge to prove that Een plus een is een.
    ...
    def create_vector_store(self, assistant_id: str, embeddings_data: List[Dict[str, Any]]) -> str:
    ...
    async def process_repository(self) -> ProcessingStats:
        ...
        self.logger.info("Starting Een repository processing for OpenAI RAG integration")
        ...
        self.logger.info(f"Processing complete: {stats.total_chunks} chunks, {stats.total_tokens} tokens")
        self.logger.info(f"Estimated embedding cost: ${stats.estimated_cost_usd:.4f}")
        
        # Check cost limit
        if stats.estimated_cost_usd > self.hard_limit_usd:
            raise ValueError(f"Estimated cost ${stats.estimated_cost_usd:.4f} exceeds limit ${self.hard_limit_usd}")
        ...
        for i in range(0, len(all_chunks), batch_size):
            ...
            self.logger.info(f"Processed embedding batch {i//batch_size + 1}/{(len(all_chunks) + batch_size - 1)//batch_size}")
        ...
        self.logger.info(f"Repository indexing complete in {stats.processing_time_seconds:.1f}s")
