...
class MasterIntegrationSystem {
    ...
    initializeUnifiedSystems() {
        ...
        // Prefer unified chatbot system; disable legacy PersistentChatSystem
        if (typeof UnifiedChatbotSystem !== 'undefined') {
            this.systems.chat = window.unifiedChatbot || new UnifiedChatbotSystem();
            console.log('  ✓ Unified chatbot system active');
        } else if (typeof PersistentChatSystem !== 'undefined') {
            // Fallback only if unified is unavailable
            this.systems.chat = new PersistentChatSystem();
            console.log('  ✓ Legacy persistent chat fallback active');
        }
